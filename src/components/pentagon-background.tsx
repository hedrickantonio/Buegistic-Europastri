...
const PentagonBackground = () => {
  useEffect(() => {
    function animateBluegisticsGlow() {
      const glow = document.querySelector<SVGPathElement>("#logo-glow");
      if (!glow) return;

      const length = glow.getTotalLength();

      // Configura dash para que el "segmento brillante" sea corto
      // Segmento visible = 12% del path, resto invisible
      const dashVisible = Math.max(60, length * 0.12);
      const dashGap = length;

      glow.style.strokeDasharray = `${dashVisible} ${dashGap}`;
